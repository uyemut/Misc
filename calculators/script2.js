	    ...
	    squareRoot(){
	        const current = parseFloat(this.currentOperand);
                ...
                if (current < 0) {
                    this.showError("Invalid Input (Negative Root)");
                    this.clear();
                    return;
                }
                this.currentOperand = (Math.round(Math.sqrt(current) * 100000000) / 100000000).toString() ;
                this.updateDisplay();
            }
        }
		...
		if (action === 'sqrt') calculator.squareRoot();
		if (action === 'compute') calculator.compute();
		if (action === 'compute') calculator.compute();
            });
        });
